...
# pem_file = os.environ['OPENAI_PEM_FILE']
...
def get_response(query:str):
    result = VisualAIResponse.graph_generator(query)
    return result
...
class VisualAIResponse:
    ...
    def get_sqlquery(text):
        graph_name = VisualAIResponse.remove_newlines(VisualAIResponse.get_graphname(text))
    ...
    def graph_generator(text):
        sql_query,Table = VisualAIResponse.get_sqlquery(text)
        if Table == "claimsDeepDive":
            result = pd.read_excel(r"C:\Users\AL56164\OneDrive - Elevance Health\Desktop\SAMPLE DATATEST.xlsx")
        else:
            result = pd.read_csv(r"C:\Users\AL56164\OneDrive - Elevance Health\Desktop\deep_dive_diag_cds.csv")
        result["COUNT"] = [2,3,4,5]
        graph_name = VisualAIResponse.get_graphname(text)
