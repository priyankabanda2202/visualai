...
def get_insight(data, query):
    ...
    template = f"""You are a polite and helpful AI assistant \
        ...
        your job is to generate the useful and detailed answer which gives the insghts from the input data in few points \
        (Note: Column names should match the keys of the dictionary i.e. {dict1})\
        written in triple backticks \
        Provide some suggestions (if any) \
    ...
    response = openai.ChatCompletion.create( model = "gpt-35-turbo-0613",   messages = [
    {
        "role": "system",
        "content": template
    }
