...
logging.basicConfig(filename='VisualAILogs.txt',format='%(asctime)s - %(name)s - %(levelname)s - %(message)s', level=logging.INFO)
...
LlmModel = ChatOpenAI(model_name="gpt-35-turbo-0613", temperature = 0)
SqlGeneratorTemplate = {
...
graph_attributes_dict = {
    # Chart 1:
    "Population_Volume" : {
        "template" : SqlGeneratorTemplate["template_1"],
...
GROUP BY claim_completion_month_adj"""
    ...
    "Trend_Analysis_Amounts" : {
        "template" : SqlGeneratorTemplate["template_2"],
...
def graph_name_script(query):
    logger.info(f"query in Graph name script: {query}")  
    ...
    logger.info(f"Graph name by the script: {graph_name_by_script}")
    return graph_name_by_script
    
    

def get_graphname_llm(query):
    logger.info("Query entered in LLM: {query}")
    template = """
    ...
    try:
        ...
        logger.info(f"Error in get_graphname_llm : {e}")
    return response_by_llm


def get_final_graphname(response_by_llm,graph_name_by_script):
    logger.info(f"Responses in the final graphname function: Response by llm --> {response_by_llm}")
    logger.info(f"Responses in the final graphname function: graph_name_by_script --> {graph_name_by_script}")
...
def get_sql_query(user_prompt, filter):
    ...
    logger.info(f"Entered the Get Sql query --> {graph_name}")
